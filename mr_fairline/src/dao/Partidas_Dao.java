...
public class Partidas_Dao extends Connect {
	...
	private static final String SELECT = "SELECT TIME_MANDANTE, "
			+ "       TIME_VISITANTE, "
			+ "       NOME_ESTADIO, "
			+ "       SIGLA_TIME_M, "
			+ "       SIGLA_TIME_V, "
			...
			+ "       HORA "
			+ " FROM "
			+ "  (SELECT time_m.nome_popular AS TIME_MANDANTE, "
			...
			+ "   INNER JOIN TIME time_v ON jg.time_visitante_id = time_v.time_id) AS DATA "
			+ " ORDER BY DATA ASC;";
